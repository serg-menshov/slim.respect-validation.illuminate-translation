...
$app->post('/user/register', function (Request $request, Response $response) {
  ...
  // and then you can use them here

  // for example return parameters as is
...
// example 2
$app->put('/user/profile', function (Request $request, Response $response) {
  $params = $request->getAttribute('validated');
  $response->getBody()->write(json_encode($params, JSON_PRETTY_PRINT));
  return $response;
...
// example 3
$app->get('/split-time-calc', function (Request $request, Response $response) {
  $params = $request->getAttribute('validated');
  $response->getBody()->write(json_encode($params, JSON_PRETTY_PRINT));
  return $response;
